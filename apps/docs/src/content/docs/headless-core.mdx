...
Zero-dependency IANA timezone support built on `Intl.DateTimeFormat`:
...
// Offset (minutes east of UTC) at a specific instant.
...
// Wall-clock ⇄ UTC. Default DST handling:
// - gap (spring-forward, wall doesn't exist)  → shift forward
// - overlap (fall-back, wall happens twice)   → earlier instant
...
See [Timezones](/timezones/) for the full guide, including DST edge cases and how `event.timeZone` plus `displayTimeZone` interact.
