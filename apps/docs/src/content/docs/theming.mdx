...
The calendar includes print-optimized CSS via `@media print`:

- The toolbar is hidden automatically
- Scrollable areas become fully visible
- Drag handles and resize handles are hidden
- Background colors are simplified for ink saving
- Event colors are preserved with `print-color-adjust: exact`

No configuration needed — just use `Ctrl+P` (or `Cmd+P` on Mac) to print.

## RTL support

trud-calendar automatically supports right-to-left layouts. Add `dir="rtl"` to a parent element:
...
The calendar flips borders, padding, and text alignment automatically. No extra configuration needed.
